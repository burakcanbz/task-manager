...
async def clean_tasks():
    async for db in get_db():
        try:
            ...
            await db.commit()
            await logger.info(f"✅ {deleted_count} tasks deleted from DB")
            return
        except Exception as e:
            await db.rollback()
            await logger.info(f"❌ Error cleaning tasks: {e}")
...
async def seed_data():
    try:
        ...
        await logger.info("Error: tasks.json file not found!")
        return
    ...
    async for db in get_db():
        try:
            ...
            if total_added > 0:
                await db.commit()
                await logger.info(
                    f"✅ Seeder finished successfully! Total {total_added} tasks added."
                )
            ...
            await db.rollback()
            await logger.info(f"❌ Error seeding data: {e}")
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                await logger.info(
                    f"Database connection failed ({attempt+1}/{max_retries}), retrying in {retry_delay}s..."
                )
...
async def set_data_to_redis(key: str, data: T, ttl: Optional[int] = None) -> bool:
    ...
    await logger.info(f"Data set to Redis: {key}")
    return True
...
async def get_data_from_redis(key: str) -> Optional[T]:
    ...
    await logger.info(f"Data retrieved from Redis: {key}")
