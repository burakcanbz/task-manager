...
from utils.utils import set_data_to_redis, get_data_from_redis, delete_from_redis, add_to_redis, update_to_redis
...
class TaskService():
    ...
    async def get_all(self) -> List[Task]:
        ...
        if tasks:
            serialized = [TaskResponse.model_validate(task).model_dump(mode="json") for task in tasks]
            await set_data_to_redis(key=redis_all_task_key, data=serialized, ttl=60*60)
    ...
    async def update_task(self, id: int, task_data: TaskUpdate) -> Task:
        task: Task = await self.repository.get_by_id(id) # also redis can be apply here to get most frequent finding task.
